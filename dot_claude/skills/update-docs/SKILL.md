...
Launch **parallel Explore agents** to gather codebase intelligence. Use at minimum these 3 agents concurrently:
...
**Agent 3 — Code conventions & patterns:**
...
**Ask the user for confirmation** using the `AskUserQuestion` tool before making any edits. Include the full structured summary above in the question so it renders inline. Accept per-file approval (e.g. "skip CLAUDE.md, do the rest").

**Exception:** If `$ARGUMENTS` contains `--yes` or `-y`, skip confirmation and apply all updates immediately.
...
5. **CLAUDE.md is special** — it contains instructions for Claude, not code docs. Update conventions and commands, don't add code explanations
6. **Parallelize scanning** — use concurrent Explore agents for speed
